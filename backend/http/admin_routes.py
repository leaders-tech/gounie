"""Handle admin-only endpoints: users, karma, bans, real deletes, and closing any bet.
...
from backend.db.bets import delete_comment, get_bet
...
from backend.http.fields import read_choice, read_id, read_int
...
async def _close_bet_as_admin(request: web.Request, status: str) -> web.Response:
    ...
    if await get_bet(request.app["db"], bet_id) is None:
